...
| AI Is Not Valuable. Your Context Window Is. | **HOLD — PARKED OUT OF THIS REPO.** Both cuts moved to `tech-news/pending-insights/held-articles/` in the private monorepo on 2026-09-20 so `scrub-check` can pass clean. They name an unfiled R&D device, which trips the `track[ -]?breaker` scrub rule by design. They come back here when patent counsel clears them, not before. | **Syndicate nothing.** Distribution plan and rewrite rules are preserved in the review notes beside the drafts. |
